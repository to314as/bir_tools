...
class ComplexConvTranspose3d(Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0,output_padding=0, groups=1, bias=True, dilation=1, padding_mode='zeros'):
        ...
        self.conv_tran_r = ConvTranspose3d(in_channels, out_channels, kernel_size, stride, padding,output_padding, groups, bias, dilation, padding_mode)
        self.conv_tran_i = ConvTranspose3d(in_channels, out_channels, kernel_size, stride, padding,output_padding, groups, bias, dilation, padding_mode)
        IF_R=torch.Tensor([[[[[np.cos(2*(j)*(i)*np.pi/kernel_size[2])/kernel_size[2] for j in range(kernel_size[2])] for i in range(kernel_size[1])]for i in range(kernel_size[0])] for i in range(out_channels)] for i in range(in_channels)]).double()
        IF_IMAG=torch.Tensor([[[[[np.sin(2*(j)*(i)*np.pi/kernel_size[2])/kernel_size[2] for j in range(kernel_size[2])] for i in range(kernel_size[1])]for i in range(kernel_size[0])] for i in range(out_channels)] for i in range(in_channels)]).double()
        print(IF_R.shape)
        #IFB=torch.Tensor([0 for i in range(out_channels)]).double()
        with torch.no_grad():
            self.conv_tran_r.weight = torch.nn.Parameter(IF_R)
